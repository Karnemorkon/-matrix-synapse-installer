...
# Створюємо базову директорію /opt/matrix якщо вона не існує
if [ ! -d "/opt" ]; then
    mkdir -p /opt
fi
...
BASE_DIR="/opt/matrix" # Базова директорія для всіх файлів Matrix
...
log_echo "  - Postgres база даних: /opt/matrix/synapse/data/postgres"
log_echo "  - Synapse медіа та ключі: /opt/matrix/synapse/data (за виключенням postgres)"
log_echo "  - Конфігурація Synapse: /opt/matrix/synapse/config"
log_echo "  - Конфігурації мостів (якщо встановлені):"
if [ "$INSTALL_SIGNAL_BRIDGE" = "yes" ]; then log_echo "    - Signal: /opt/matrix/signal-bridge/config та /opt/matrix/signal-bridge/data"; fi
if [ "$INSTALL_WHATSAPP_BRIDGE" = "yes" ]; then log_echo "    - WhatsApp: /opt/matrix/whatsapp-bridge/config та /opt/matrix/whatsapp-bridge/data"; fi
if [ "$INSTALL_TELEGRAM_BRIDGE" = "yes" ]; then log_echo "    - Telegram: /opt/matrix/telegram-bridge/config та /opt/matrix/telegram-bridge/data"; fi
if [ "$INSTALL_DISCORD_BRIDGE" = "yes" ]; then log_echo "    - Discord: /opt/matrix/discord-bridge/config та /opt/matrix/discord-bridge/data"; fi
log_echo "  - Файл docker-compose.yml: /opt/matrix/docker-compose.yml"
